...
pub fn inline_primitives<'a>(ctx: &mut MIRContext<'a>) -> bool {
            ...
            &mut |statement, scope| {
                ...
                if let Some(value) = value {
                    ...
                    if matches!(
                        ...
                        // to within it.
                        // TODO: Allow inlining a variable when that variable is a primitive type.
                ...
                if !find_exprs(statement, &mut |expr| {
                ...
                // We don't want to modify SetVariable for two reasons:
                // - This might inline in the new value by accident.
                // - SetVariable includes place expressions, which we might accidentally interpret as variable reads
                //   and try to inline. This is a bit of a fragile way to handle this, but SetVariable is the only place
                //   where we set from a place expression.
                if !matches!(statement, MIRStatement::SetVariable { .. })
